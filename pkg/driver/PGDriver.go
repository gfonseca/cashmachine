...
func formatURL(host string, port int, user, pass, db string) string {
	s := fmt.Sprintf("postgresql://%s:%d/%s?user=%s&password=%s", host, port, db, user, pass)
	logrus.Info(s)
	return s
}
...
func (pgd *PGDriver) Get(id int) (balance float32, err error) {
	q := "SELECT balance FROM cashmachine_schema.accounts WHERE id=$1"
...
func (pgd *PGDriver) Update(id int, value float32) (err error) {
	q := "UPDATE cashmachine_schema.accounts SET balance=$1 WHERE id=$2"
...
func (pgd *PGDriver) Create(value float32) (id int, err error) {
	q := "INSERT INTO cashmachine_schema.accounts (balance) VALUES ($1) RETURNING(id)"
